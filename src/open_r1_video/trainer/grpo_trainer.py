...
class Qwen25VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if "image" in inputs[0] and inputs[0]['image'] != None:
            from PIL import Image
            print(inputs[0]['image'])
            inputs[0]['image'] = Image.open(inputs[0]['image'])

        if "image" in inputs[0] and inputs[0]['image'] != None:
            inputs[0]["prompt"][0]["content"][0]["type"] = "image"
        elif "video" in inputs[0] and inputs[0]['video'] != None:
            inputs[0]["prompt"][0]["content"][0]["type"] = "video"
        ...
        prompt_inputs = self.processing_class(
            text=prompts_text,
            images=images if ("image" in inputs[0] and inputs[0]["image"] is not None) else None,
            videos=video_inputs if ("video" in inputs[0] and inputs[0]["video"] is not None) else None,
            return_tensors="pt",
            padding=True,
            padding_side="left",
